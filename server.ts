...
async function configureDevelopment(app: Hono): Promise<ViteDevServer> {
  ...
  app.use("*", async (c, next) => {
    ...
    try {
      ...
      // Preserve the query string (e.g. ?raw, ?url, ?worker) so Vite applies
      // the right transform. c.req.path drops it, which blanks such imports.
      const search = new URL(c.req.url).search;

      let result;
      try {
        result = await vite.transformRequest(url + search);
      } catch {
        result = null;
      }
